...
<section class="gradient-hero section-padding pb-14">
    <div class="container-narrow reveal-on-scroll">
        ...
        <h1 class="mt-4 font-display text-4xl font-semibold text-white sm:text-5xl">Resources for Business & Tech </h1>
        <p class="mt-4 max-w-2xl text-lg text-slate-300">Practical guides on web development, AI, software, and technology.</p>

        <form method="get" action="{{ route('blog.index') }}" role="search" class="mt-8 flex max-w-md gap-2">
            <label for="blog-search" class="sr-only">Search blogs, guides, or topics...</label>
            <input id="blog-search" type="search" name="q" value="{{ $search }}" placeholder="Search blogs, guides, or topics..."
...
<section class="section-padding">
    <div class="container-narrow">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-soft">
            <p class="font-display text-lg font-semibold text-navy">No blog posts found</p>
            <p class="mt-2 text-sm text-slate-500">
                @if($search !== '')
                Nothing matched “{{ $search }}”. Try a different keyword or <a href="{{ route('blog.index') }}" class="font-semibold text-brand-700 hover:text-brand-800">browse all blog posts</a>.
                @else
                New blog posts are on the way. Check back soon.
                @endif
            </p>
        </div>
        @else
